...
(eval '
 (define (partial-sums op s) ;以s为参数,返回的流是 s0,(op s0 s1),(op s0 s1 s2), ...
   (define (stream-map fun s)
     (if (stream-null? s)
         the-empty-stream
         (cons-stream (fun (stream-car s)) (stream-map fun (stream-cdr s)))))
   (define (lst str)
     (cons-stream (list (stream-car s)) (cons (stream-car s) (lst (stream-cdr s)))))
   (stream-map (lambda (x) (apply op x)) (lst s)))
env)
(define (myloop)
