...
(define (ripple-carry-adder wire-lstA wire-lstB wire-lstS wireCN)
  (define (myadder lstA lstB lstS Cin CN)
    (if (null? lstA)
        (or-gate Cin Cin CN)
        (let ((Cout (make-wire)))
          (begin
            (full-adder (car lstA) (car lstB) Cin (car lstS) Cout)
            (myadder (cdr lstA) (cdr lstB) (cdr lstS) Cout CN)))))
  (let ((a (make-wire)))
    (begin
      (set-signal! a 0)
      (myadder wire-lstA wire-lstB wire-lstS a wireCN))))
