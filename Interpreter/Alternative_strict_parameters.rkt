...
;basic normal order scheme interpreter in text book, with "let", "and", "or" implemented,
;but with no support to cons, car,cdr.
...
(define (thunk? obj)
  (mtagged-list? obj 'thunk))

(define (delay-it-memo exp env) 
  (mlist 'thunk-memo exp env))

(define (thunk-memo? obj)
  (mtagged-list? obj 'thunk-memo))
  ...
  (cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
         ...
         (eval-sequence
          ...
          (extend-environment  
           (list->mlist (map (lambda (x) (if (symbol? x) x (car x))) (procedure-parameters procedure)))
           (list->mlist (list-of-typed-args (procedure-parameters procedure) arguments env))
...
(define (list-of-typed-args args exps env) 
  (if (no-operands? exps)
      '()
      (cons (cond ((symbol? (car args)) (actual-value (first-operand exps) env))
                  ((eq? (cadar args) 'lazy) (delay-it (first-operand exps) env))
                  ((eq? (cadar args) 'lazy-memo) (delay-it-memo (first-operand exps) env)))
            (list-of-typed-args (cdr args)
                                (rest-operands exps)
                                  env))))
