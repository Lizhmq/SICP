...
;基于Scheme解释器的实现进行一些修改，实现惰性求值的解释器
;将cons,car,cdr实现为惰性后列表自然成为流

;实现记忆化的槽
;根据槽需要强迫的三种情况,修改my-apply,eval-if,eval-and,eval-or
;my-apply需要有对应的参数惰性化
;注意到输入的形如'(a b c)的list中需要人为地转化为惰性cons对，否则car,cdr出错
;于是要修改quote-text
...
(define (my-eval exp env)
  (cond ((self-evaluating? exp) exp)                            ;自求值表达式
        ...
        ((quoted? exp) (text-of-quotation exp env))                 ;单引号表达式(符号)
        ...
        ((application? exp)                                     ;apply过程用actual-value求真值
...
(define (my-apply procedure arguments env)                 ;过程apply时在新的内层环境中求值
        ...
        ((compound-procedure? procedure)
         (eval-sequence
          ...
          (extend-environment
           ...
           (list-of-delayed-args arguments env)            ;参数设为惰性求值
...
(define (driver-loop)
  (let ((input (read)))
    (if (eq? input eof)
        (void)
        (begin
          (let ((output (actual-value input glb-env)))    ;输出设置为真实值
...
(define glb-env (setup-environment))        ;惰性实现的cons,car,cdr，那么任意的列表都相当于流
(void (actual-value '(define (cons x y) (lambda (f) (f x y))) glb-env))
(void (actual-value '(define (car z) (z (lambda (x y) x))) glb-env))
(void (actual-value '(define (cdr z) (z (lambda (x y) y))) glb-env))
(void (actual-value '(define (cadr z) (car (cdr z))) glb-env))
;将大括号转换为小括号
